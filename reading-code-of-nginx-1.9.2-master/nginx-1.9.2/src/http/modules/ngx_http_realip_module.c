...
//怎样在NGX_HTTP_POST_READ_PHASE或者NGX_HTTP_PREACCESS_PHASE阶段添加HTTP模块
static ngx_int_t
ngx_http_realip_init(ngx_conf_t *cf)
{
    ...
    /*
     phases数组中有11个成员，取出NGX HTTP POST READ PHASE阶段的handlers动态数组，向其中添加ngx_http_handler_pt处理方法，这
     样ngx_http_realip_module模块就介入HTTP请求的NGX_HTTP_POST_READ_PHASE处理阶段了
     */
    ...
    *h = ngx_http_realip_handler;
    ...
    /*
    实际上，同一个HTTP模块的同一个ngx_ht tp_realip_handler方法，完全可以设置到两个不同的阶段中的。例如，phases[NGX_HTTP_PREACCESS_PHASE.handlers]
    动态数组中也添加了ngx_http_realip_handler方洪
     */
