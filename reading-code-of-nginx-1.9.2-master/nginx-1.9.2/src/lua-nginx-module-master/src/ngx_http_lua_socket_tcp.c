    ...
    ngx_http_lua_socket_tcp_upstream_t *u, lua_State *L)
{
    ...
    ngx_add_timer(c->write, u->connect_timeout, NGX_FUNC_LINE);
...
new_ssl_name:
    ...
    if (rc == NGX_AGAIN) {
        ...
        ngx_add_timer(c->read, u->connect_timeout, NGX_FUNC_LINE);
...
success:
    ...
    if (rev->active) {
        ngx_add_timer(rev, u->read_timeout, NGX_FUNC_LINE);
    ...
    ngx_http_lua_socket_tcp_upstream_t *u)
{
    ...
    ngx_add_timer(c->write, u->send_timeout, NGX_FUNC_LINE);
    ...
    ngx_http_lua_socket_tcp_upstream_t *u)
{
    ...
    if (u->raw_downstream || u->body_downstream) {
        if (r->connection->read->timer_set) {
            ngx_del_timer(r->connection->read, NGX_FUNC_LINE);
        }
        return;
    }
    ...
    if (u->raw_downstream || u->body_downstream) {
        if (ctx && ctx->writing_raw_req_socket) {
            ...
            if (r->connection->write->timer_set) {
                ngx_del_timer(r->connection->write, NGX_FUNC_LINE);
            }
...
ngx_http_lua_socket_tcp_setkeepalive(lua_State *L)
{
    ...
    if (timeout) {
        ...
        ngx_add_timer(c->read, timeout, NGX_FUNC_LINE);
    }
